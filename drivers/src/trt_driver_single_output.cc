...
void *init_engine(config &_config)
{
  ...
  try
  {
    ...
    // const auto total_bindings = driver->m_engine->getNbBindings();
...
// https://github.com/cyrusbehr/tensorrt-cpp-api/blob/main/include/engine/EngineRunInference.inl

int __run_trt_infer(_oris_orl_trt_driver *_driver, std::vector<cv::cuda::GpuMat> &input, std::vector<float> &result)
{
#ifdef DEV
  const auto numInputs = _driver->m_inputDims.size();

  std::cerr << "NumInputs is " << numInputs << std::endl;

  if (input.size() != numInputs)
  {
    throw std::runtime_error("Error, Incorrect number of inputs provided!");
  }
  ...
  {
    const auto &dims = _driver->m_inputDims[i];
    auto &d_in = input[i];
    if (d_in.channels() != dims.d[0] || d_in.rows != dims.d[1] || d_in.cols != dims.d[2])
    {
      throw std::runtime_error("Error, Input does not have correct size!");
    }

    nvinfer1::Dims4 inputDims = {batchSize, dims.d[0], dims.d[1], dims.d[2]};
    _driver->m_context->setInputShape(_driver->m_IOTensorNames[i].c_str(),
...
int _detection_func(_oris_orl_trt_driver *_driver, cv::Mat &imgin, std::vector<Tensor> &result)
{
  int ret = 0;
#ifdef DEV
  ...
  try
  {
    ...
    size_t height = _driver->m_inputDims[0].d[1];
    size_t width = _driver->m_inputDims[0].d[2];

    _driver->m_ratio = _img_preprocess(imgin, converted_img, height, width);

    cv::cuda::GpuMat gpuImg;
    gpuImg.upload(converted_img);

    std::vector<cv::cuda::GpuMat> input{std::move(gpuImg)}; // no batch

    std::vector<float> features;

    ret = __run_trt_infer(_driver, input, features);
  }
  catch (std::runtime_error &ex)
  {
  }
...
void _detection_func(_oris_orl_trt_driver *driver, float *input, std::vector<Tensor> &result)
{
  cudaStream_t inferenceCudaStream;
  _check_cuda_error_code(cudaStreamCreate(&inferenceCudaStream));

  _check_cuda_error_code(cudaMemcpyAsync(driver->m_bindings[0], input, driver->m_binding_sizes[0], cudaMemcpyHostToDevice, inferenceCudaStream));
  ...
  Tensor resultItem;
  auto outdims = driver->m_output_dims.begin();
  resultItem = makeTensor(DataType::FLOAT32, outdims->second);
  auto output_vec = dynamic_cast<vector_t<float> *>(resultItem.m_data.get());
  auto &dataptr = output_vec->get();

  _check_cuda_error_code(cudaMemcpyAsync(dataptr.data(), driver->m_bindings[1], driver->m_binding_sizes[1], cudaMemcpyDeviceToHost, inferenceCudaStream));
...
void _trt_run_infer(_oris_orl_trt_driver *driver, const std::vector<Tensor> &input, std::vector<Tensor> &result)
{
  ...
  if (driver->m_output_bindings.size() > 1) // 
  {
  ...
  /*size_t outsize = oris::get_volume_size(driver->m_output_dims[output_name]);
  std::vector<float> output_buffer(outsize);
  ...
  std::cerr << "OUT DIMS " << std::endl;
  std::vector<size_t> &outdims = driver->m_output_dims[output_name];
...
*/
  ...
  std::vector<size_t> tensor_single_dims;
